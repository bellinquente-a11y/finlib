...
def test_convert_binance_data_to_DataFrame_symbol_data_missing() -> None:
    assert (
        (
            _convert_binance_data_to_DataFrame({"SYM1": [_EX_ROW_BROW], "SYM2": None})
            == _convert_binance_data_to_DataFrame({"SYM1": [_EX_ROW_BROW]})
        )
        .all()
        .all()
    )
...
async def test_validated_fetch_binance_one_symbol_invalid_rows_count() -> None:
    ...
    async def malformed_rows(
        session: aiohttp.ClientSession, symbol: str, interval: binance_interval, limit: int
    ) -> list[list[Any]]:
        return [_EX_ROW, _EX_ROW[1:], _EX_ROW[1:], _EX_ROW, _EX_ROW]

    with patch(
        "finlib.async_fetch._fetch_binance_one_symbol_with_retry", new_callable=AsyncMock
    ) as mock:
        mock.side_effect = malformed_rows
        with capture_logs() as logs:
            _ = await _validated_fetch_binance_one_symbol(
                aiohttp.ClientSession(), "AAA", "1m", 1, semaphore
            )
...
async def test_validated_fetch_binance_one_symbol_invalid_rows_remaining_output() -> None:
    ...
    with patch(
        ...
        mock.side_effect = malformed_rows
        result = await _validated_fetch_binance_one_symbol(
            aiohttp.ClientSession(), "AAA", "1m", 1, semaphore
        )
