...
@settings(max_examples=MAX_HYPOTHESIS_SAMPLES)
@given(b=st.floats(min_value=0, exclude_min=True), cap=st.floats(min_value=0, exclude_min=True))
...
def test_kelly_criterion_monotone_non_decreasing_in_p_at_fixed_b(
    p1: float, p2: float, b: float, cap: float
) -> None:
...
@settings(max_examples=MAX_HYPOTHESIS_SAMPLES)
@given(inputs=p_b_resulting_in_negative_edge(), cap=st.floats(min_value=0, exclude_min=True))
