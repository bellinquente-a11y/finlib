...
async def _fetch_binance_one_symbol(session: aiohttp.ClientSession, 
    ...
    async with asyncio.timeout(settings.fetch_timeout_seconds):
        async with session.get(settings.binance.url, 
...
@async_retry(max_retry=3, delay=1., exceptions=(aiohttp.ClientError, asyncio.TimeoutError))
async def _fetch_binance_one_symbol_with_retry(session: aiohttp.ClientSession, 
                                               symbol: str, 
                                               interval: binance_interval, 
                                               limit: int) -> Any:
...
async def _validated_fetch_binance_one_symbol(session: aiohttp.ClientSession, 
                                              symbol: str, 
                                              interval: binance_interval, 
                                              limit: int, 
    ...
    async with semaphore:
        try:
            ...
            log.warning("Unable to fetch Binance data", 
                        url=settings.binance.url, symbol=symbol, exc=e)
            return None

async def _fetch_binance_raw_data(symbols: list[str], 
                                  interval: binance_interval, 
                                  limit: int) -> dict[str, list[BinanceDataRow] | None]:
...
async def fetch_binance(symbols: list[str], 
                        interval: binance_interval, 
                        start: datetime | str) -> pd.DataFrame:
    match interval[-1]:
        case "s":
